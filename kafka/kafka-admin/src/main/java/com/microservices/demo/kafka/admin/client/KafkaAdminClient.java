...
public class KafkaAdminClient {
    ...
    public void checkSchemaRegistry(){
        ...
        Integer multiplier = retryConfigData.getMultiplier().intValue();
        Long sleepTimeMs = retryConfigData.getSleepTimeMs();
        while (!getSchemaResitryStatus().is2xxSuccessful()){
            checkMaxRetry(retryCount++, maxRetry);
            sleep(sleepTimeMs);
            sleepTimeMs *= multiplier;
        }
    ...
    private HttpStatus getSchemaResitryStatus(){
        try {
    ...
    public Collection<TopicListing> getTopics(){
        ...
        try{
            ...
            throw new KafkaClientException("Reached max number of retry to get topics!");
        }
        return topics;
    }

    public Collection<TopicListing> doGetTopics(RetryContext retryContext) throws ExecutionException, InterruptedException {
        LOG.info("Reading Kafka topic {}, attmpt {}", kafkaConfigData.getTopicNamesToCreate().toArray(), retryContext.getRetryCount());
